...
Active non-mono border and line variants render their dynamic color layer with vgpu/WebGPU. The WGSL shader is generated from the same upstream-derived palette data as the CSS. Existing CSS masks, geometry, opacity, pulse driver, and static palettes remain the source of truth for shape and fallback behavior.
...
- `scripts/generate-css.mjs` remains the CSS source of truth. Generated `border-beam.css` disables hue-shift CSS animations once `data-vgpu-colors` is active.
